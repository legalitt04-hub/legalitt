...
export default function FIRDrafts() {
  const [cases, setCases] = useState<FIRDraft[]>([]);
  ...
  // Modals
  const [selectedCase, setSelectedCase] = useState<FIRDraft | null>(null);
  const [editCase, setEditCase] = useState<FIRDraft | null>(null);
  const [deleteId, setDeleteId] = useState<string | null>(null);
  const [uploadModalCase, setUploadModalCase] = useState<FIRDraft | null>(null);
  ...
  const fetchCases = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await api.get('/admin/fir-drafts', { params });
      setCases(data.data || []);
  ...
  const handleSaveEdit = async () => {
    if (!editCase) return;
    try {
      await api.put(`/admin/fir-drafts/${editCase._id}/status`, {
  ...
  const handleUploadCaseDoc = async () => {
    ...
    try {
      ...
      await api.post(`/admin/fir-drafts/${uploadModalCase._id}/upload`, formData, {
  ...
  const handleDelete = async (id: string) => {
    try {
      await api.delete(`/admin/fir-drafts/${id}`);
      setDeleteId(null);
      fetchCases();
  ...
  const kanbanData = KANBAN_COLS.reduce<Record<string, FIRDraft[]>>((acc, col) => {
    acc[col] = filteredCases.filter(c => c.status === col);
    return acc;
  }, {});

  const CaseCard = ({ c }: { c: FIRDraft }) => {
