...
export default function DocumentForensic() {
  const [cases, setCases] = useState<DocumentForensic[]>([]);
  ...
  // Modals
  const [selectedCase, setSelectedCase] = useState<DocumentForensic | null>(null);
  const [editCase, setEditCase] = useState<DocumentForensic | null>(null);
  const [deleteId, setDeleteId] = useState<string | null>(null);
  const [uploadModalCase, setUploadModalCase] = useState<DocumentForensic | null>(null);
  ...
  const fetchCases = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await api.get('/admin/document-forensic', { params });
      setCases(data.data || []);
  ...
  const handleSaveEdit = async () => {
    if (!editCase) return;
    try {
      await api.put(`/admin/document-forensic/${editCase._id}/status`, {
  ...
  const handleUploadCaseDoc = async () => {
    ...
    try {
      ...
      await api.post(`/admin/document-forensic/${uploadModalCase._id}/upload`, formData, {
  ...
  const handleDelete = async (id: string) => {
    try {
      await api.delete(`/admin/document-forensic/${id}`);
      setDeleteId(null);
      fetchCases();
  ...
  const kanbanData = KANBAN_COLS.reduce<Record<string, DocumentForensic[]>>((acc, col) => {
    acc[col] = filteredCases.filter(c => c.status === col);
    return acc;
  }, {});

  const CaseCard = ({ c }: { c: DocumentForensic }) => {
