...
const Sidebar = ({ isOpen, setIsOpen }: SidebarProps) => {
  ...
  return (
    <>
      ...
      <aside 
        className={cn(
          "w-64 fixed h-screen top-0 left-0 p-4 z-50 transition-transform duration-300 ease-in-out lg:translate-x-0",
          isOpen ? "translate-x-0" : "-translate-x-full"
        )}
      >
        <div className="h-full w-full bg-slate-900/95 backdrop-blur-xl border border-slate-800 rounded-[24px] flex flex-col shadow-2xl overflow-hidden relative overflow-y-auto hidden-scrollbar">
          {/* Logo */}
