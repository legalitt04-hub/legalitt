...
async function seedBookingsMain() {
  try {
    ...
    // 2. Find ONLY the 6 seeded advocates (emails ending in @legalitt-seed.com)
    // Plus any custom advocate accounts if we want, but these 6 are the main ones.
    const advocateUsers = await User.find({ 
      role: 'advocate',
      email: { $regex: /@legalitt-seed\.com$/i }
    }).lean();

    console.log(`Found ${advocateUsers.length} main seeded advocate accounts in the database.`);
    ...
    if (advocates.length === 0) {
      console.log('❌ Seeding failed: No Advocate profiles found for the main seeded advocates.');
      process.exit(1);
    }
    ...
    console.log('🧹 Cleared existing bookings for these main seeded advocates.');
    ...
    console.log('✅ Success! Bookings and paid transactions seeded successfully for main seeded advocates.');
    process.exit(0);
  } catch (error) {
