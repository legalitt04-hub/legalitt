...
async function seedBookings() {
  try {
    ...
    if (!clientUser) {
      console.log('❌ Seeding failed: Seed client not found. Please run seed-client.js first!');
      process.exit(1);
    }

    // 2. Get all seeded advocates
    const advocates = await Advocate.find().populate('user');
    if (advocates.length === 0) {
      console.log('❌ Seeding failed: Seed advocates not found. Please run npm run seed first!');
      process.exit(1);
    }

    console.log(`Found ${advocates.length} advocates to seed bookings for.`);

    // Clear existing bookings for these seed advocates to start fresh
    const advocateIds = advocates.map(a => a._id);
    await Booking.deleteMany({ advocate: { $in: advocateIds } });
    console.log('🧹 Cleared existing bookings for seed advocates.');

    const now = new Date();

    // 3. Loop through advocates and create custom paid consultations
    for (const adv of advocates) {
      console.log(`Seeding bookings for Advocate: ${adv.user?.name}`);
    ...
    console.log('✅ Success! Bookings and paid transactions seeded successfully for all advocates.');
    process.exit(0);
  } catch (error) {
